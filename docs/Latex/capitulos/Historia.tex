...
Las interrupciones son un mecanismo clave que permite la interacción y sincronización entre el hardware y el software. Se trata de eventos específicos que detienen temporalmente la ejecución del programa principal y activan una rutina especial de código para gestionar un evento particular, como el refresco de pantalla, la lectura de botones, o el temporizador. Estas interrupciones permiten que la Game Boy gestione tareas cruciales en segundo plano sin que el programa principal tenga que supervisarlas constantemente.
\\\\
Existen los distintos tipos:

\begin{itemize}
    \item \textbf{VBlank}: Como indica el nombre, esta interrupción se lanza cada vez que la PPU entra en el modo 1, también conocido como VBlank.
    \item \textbf{LCD STAT}: Hay distintas maneras de que esta interrupción se active, todas ellas relacionadas con los estados del barrido de pantalla.
    \item 
\end{itemize}
